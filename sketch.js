...
function draw() {
    if (gameRunning) {
        ...
        text('Total Clicks: ' + totalClicks, 20, 70); // Display the total clicks
    ...
    text('Time: ' + minutes + ':' + seconds, width - 150, 50); // Display timer at top right
}
...
function Boid(x, y) {
    ...
    this.maxforce = 0.05;
    this.clicked = false;
    this.isPink = true; // Make all boids pink and clickable
}
...
Boid.prototype.separate = function (boids) {
    let desiredSeparation = 50.0; // Minimum separation distance
...
Boid.prototype.cohesion = function (boids) {
    let neighbordist = 80; // Maximum distance to consider a boid as a neighbor
    let sum = createVector(0, 0);
    let count = 0;
